...
class MeshAccumulator {
    ...
    public constructor(mesh: Mesh, collectIndices: boolean, scene: Scene) {
        //<todo.eoin Do we really need to set force=true here? (all over this file)
        const worldFromRoot = mesh.computeWorldMatrix(true);
        this._rootFromWorld = Matrix.Invert(worldFromRoot);
     ...
     */
    public addMesh(mesh: TransformNode, includeChildren: boolean): void {
        const worldFromMesh = mesh.computeWorldMatrix(true);
        const meshFromRoot = worldFromMesh.multiply(this._rootFromWorld);

        var addedMesh = false;
        if (mesh instanceof Mesh) {
            this.addConcreteMesh(mesh, meshFromRoot)
            addedMesh = true;
        } else if (mesh instanceof InstancedMesh){
            this.addConcreteMesh(mesh.sourceMesh, meshFromRoot);
            addedMesh = true;
        }
        ...
        if (includeChildren) {
            ...
            // Could be a transform node containing multiple mesh instances; this structure seems
            // to be generated by the glTF loader when an instanced mesh is composed of multiple
            // render materials.
            for (const child of mesh.getChildMeshes()) {
                if (child instanceof InstancedMesh) {
                    this.addConcreteMesh(child.sourceMesh, meshFromRoot);
                }
            }
        }
    }

    private addConcreteMesh(mesh: Mesh, meshFromRoot : Matrix) : void{
        ...
        for (let v = 0; v < numVerts; v++) {
            ...
            this._vertices.push(Vector3.TransformCoordinates(pos, meshFromRoot));
        }
